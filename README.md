...
The server is written in Java using the [Dropwizard framework](https://www.dropwizard.io/en/stable/). The project contains a minimal web application that runs code to access CrateDB when called on various endpoints, and also serves static files from the `src/main/resources/assets` folder.

Database access is handled using the [PostgreSQL JDBC driver](https://jdbc.postgresql.org/).
